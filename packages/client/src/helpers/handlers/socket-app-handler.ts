...
export const startHandlingAppSocket = (socket: Socket, dispatch: Dispatch) => {
	...
	socket.on(String(Shared.Enums.SocketChannels.APP_ON_GET_PORTION_GAME_SERVERS), (servers: Shared.Interfaces.ILobbyServer[]) => {
		const lobbyServers: LobbyTypes.LobbyServerType[] = [];

		for (let server of servers) 
			lobbyServers.push({ ...server, isConnected: false, rooms: [], ping: null });
