...
interface IPlayers {
    [color: number]: {
        ...
        player: PlayerEntity;
        cache: {
            /** Последняя сохраненная позиция по X. */
            x: number;
            /** Последняя сохраненная позиция по Y. */
            y: number;
            /** Последнее сохраненное направление движения. */
            direction: number;
        }
    };
}
...
export default class PlayersContainer extends BaseContainer<PlayerEntity> {
    ...
    update(state: Shared.Interfaces.IGameState) {
    ...
    private _updatePlayers(players: Shared.Interfaces.IGameStatePlayers) {
        for (let color in players) {
